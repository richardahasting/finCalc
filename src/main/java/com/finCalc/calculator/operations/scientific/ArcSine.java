...
/**
 * Arcsine operation: pops one operand, pushes asin(x) in radians.
 *
 * <p>Stack transformation: {@code [... x ASIN]} → {@code [... asin(x)]}
 *
 * <p><strong>Note:</strong> Domain is [-1, 1]. Result in radians. Uses double internally for calculation.
 */
public enum ArcSine implements Calculation {
    INSTANCE;
    ...
    public Stack<StackItem> execute(Stack<StackItem> stack) {
        if (stack.size() < REQUIRED_OPERANDS) {
            stack.push(Error.insufficientOperands("ASIN", REQUIRED_OPERANDS, stack.size()));
            return stack;
        }
        ...
        if (operand instanceof BigNumber bn) {
            ...
            if (value.compareTo(new BigDecimal("-1")) < 0 || value.compareTo(BigDecimal.ONE) > 0) {
                stack.push(Error.domainError("ASIN", "input must be in range [-1, 1]"));
            } else {
                double result = Math.asin(value.doubleValue());
                stack.push(BigNumber.of(result));
            }
        } else {
            stack.push(new Error("ASIN requires numeric operand"));
        }
